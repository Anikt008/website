  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 h-full flex items-center justify-between gap-8">
        <Link to="/" className="text-xl lg:text-2xl font-black tracking-tighter text-slate-900 group shrink-0">
          ANIKET<span className="text-slate-300 font-light group-hover:text-slate-900 transition-colors duration-500">KUMAR</span>
        </Link>

        {/* Desktop Nav - Adjusted spacing for tablet (md) vs desktop (lg) */}
        <div className="hidden md:flex items-center md:space-x-6 lg:space-x-12">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`text-[12px] lg:text-[13px] font-bold uppercase tracking-[0.2em] transition-all hover:text-slate-900 relative py-1 ${
          ...
          <Link
            to="/contact"
            className="px-6 lg:px-8 py-3 bg-slate-900 text-white text-[12px] lg:text-[13px] font-black uppercase tracking-widest rounded-xl hover:bg-black transition-all active:scale-95 flex items-center gap-2 shadow-xl shadow-slate-200 shrink-0"
          >
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden p-3 text-slate-900 bg-slate-50 rounded-xl"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white z-[60] flex flex-col p-10 lg:p-12"
          >
            ...
            <div className="mt-auto space-y-10">
               ...
               <Link
                to="/contact"
                onClick={() => setIsOpen(false)}
                className="text-xl lg:text-2xl font-bold flex items-center gap-4 group"
               >
